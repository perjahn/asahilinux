...
void tps6598x_displayport_update_dp_sid(struct tps6598x *tps)
{
	...
	ret = tps6598x_displayport_register_partner(
		tps, le32_to_cpu(status.dp_mode));
	if (ret) {
		dev_err(tps->dev, "failed to register DP partner: %d\n", ret);
		return;
	}

	if (tps->dp_state != TPS_DPSTATE_ACTIVE)
		return;
...
static int tps6598x_displayport_enter(struct typec_altmode *alt, u32 *vdo)
{
	...
	int svdm_version, ret;

	/*
	 * Note that we don't actually support changing the mode. The controller
	 * has already entered the mode here and we just go through the motions
	 * to make the partner driver and the mux aware of this.
	 */
	mutex_lock(&tps->dp_lock);
	if (tps->dp_state != TPS_DPSTATE_IDLE) {
	...
	/* see tps6598x_displayport_notify */
	if (tps->cd321x)
	...
	if (schedule_work(&tps->dp_work)) {
		tps->dp_state = TPS_DPSTATE_ACKED_ENTER_MODE;
		ret = 0;
	} else {
		ret = -EBUSY;
	}
...
static int tps6598x_displayport_notify(struct typec_altmode *alt,
				       unsigned long conf, void *data)
{
	struct tps6598x *tps = typec_altmode_get_drvdata(alt);

	/*
	 * Apple Silicon is a special kind of broken and requires a transition
	 * to USB_ROLE_NONE which will bring down dwc3 whenever the mux mode
	 * is changed to safe state. Once the mux is switched to the real
	 * altmode we then need to switch to USB_ROLE_HOST to bring dwc3 back
	 * up.
	 */
	if (tps->cd321x) {
		if (conf == TYPEC_STATE_SAFE)
			usb_role_switch_set_role(tps->role_sw, USB_ROLE_NONE);
		else
				    ...
				    const u32 *data, int count)
{
	...
	case DP_CMD_CONFIGURE:
		...
		tps->dp_state = TPS_DPSTATE_ACTIVE;
		break;
	default:
