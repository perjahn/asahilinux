...
static int macsmc_power_event(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	if ((event & 0xffffff00) == 0x71010100) {
		...
		bool val = (event & 0xff00) != 0;

		/*
		 * This seems to happen when charging is inhibited/deinhibited,
		 * or when charger status changes?
		 */
		dev_info(power->dev, "Charger state event: %d\n", val);
