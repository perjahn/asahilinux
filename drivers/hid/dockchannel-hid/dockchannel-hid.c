...
struct dchid_stm_id {
	...
	/*
	 * Strings follow, but unclear if properly described.
	 * We grab the serial using a separate report instead.
	 */
} __packed;
...
dchid_get_interface(struct dockchannel_hid *dchid, int index, const char *name)
{
	...
	ret = of_property_read_u32(iface->of_node, "apple,hid-country-code", &val);
	if (!ret)
		iface->country_code = val;
...
static void dchid_handle_ready(struct dockchannel_hid *dchid, void *data, size_t length)
{
	...
	hid->country = iface->country_code;
